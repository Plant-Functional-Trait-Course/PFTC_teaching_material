...
Let us now shrink the dataset to make it easier to handle.
In addition, we will introduce a couple of errors that we want to fix afterwards.

The code to do this is hidden.
But if you want to replicate the introduction to the errors you can find the code here.
...
raw_traits <- raw_traits |> 
  ...
  mutate(Date = as.character(Date),
         ...
         # giant leaf
         Value = if_else(ID == "ANH3472" & Trait == "Leaf_Area_cm2", 100, Value),
...
This will allow you to git the dataset a first rough check.
The dataset has `r dim(raw_traits)[1]` observations and `r length(raw_traits)` columns.
...
The next thing to check is if the variables have the right data type/class.
For each variable you can see what type of formate it has, _dbl_ (numeric or integer), _chr_ (character), or _date_ (date) are the most common data types.
...
Now check if the variables are in the right format?
We see that the first variable *Date* is a character, which is not the right format.
This probably means that one of the dates is not a date.
Let us check all the different dates in the variable *Date*.
...
We can now see that there are 6 distinct dates in the dataset.
And one of the dates is "18", which is what caused the problem to not recognize this variable as a date.

The next step is to check which observation was the problem or if there are several.
For this we can use the function `filter()` to extract all observations with a date 18.
...
We can see that it is a single observation that has the wrong date.
The next step is to check the raw data, notes, photos, etc. to find the correct date for this observations.
It is important to keep the raw data sheets, or take a photo of them to be able to check these problems afterwards.
...
For this we will use the function `mutate()` which can be used to manipulate a column.
...
We can see that there is one duplicate entry which is exactly the same.
...
When getting a duplicate, these different options for why there is a duplicate have to be considered and carefully checked with the raw data.
...
Log transform growth variables:
...
Ops chicken foot! What happened here?
