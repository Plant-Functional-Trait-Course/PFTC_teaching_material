...
Missing data can also be found with the `validator` package, but it's a bit tedious to write one rule (`is.na()` or `!is.na()`) for each variable.
In some cases it might be useful to use, because it let's you use functions like any() or all() to 


## Check specific values

One of the things we want to check is if the unique leaf IDs are valid.
For this, we need to get a list of these IDs, using the `get_PFTC_envelope_codes` function from the PFTCFunctions package.
